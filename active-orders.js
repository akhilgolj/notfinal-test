document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchOrders();
    displayReservations(); // Display reservations on load
    ...
    function displayOrders(orders, container, showTracker = false) {
                    ...
                    <div class="order-tracker">
                        ...
                        <div class="row justify-content-between top">
                            ...
                            <div class="row d-flex icon-content">
                                ...
                                <i class="icon fa fa-concierge-bell"></i> <!-- Changed icon to a cooking pan -->
                                <div class="d-flex flex-column">
                                    <p class="font-weight-bold"> In the<br>&nbspKitchen</p>
                                </div>
                            </div>
                            <div class="row d-flex icon-content">
                                <i class="icon fa fa-road"></i>
                                <div class="d-flex flex-column">
                                    <p class="font-weight-bold">On the Way<br></p>
                                </div>
                            </div>
                            <div class="row d-flex icon-content">
        ...
        container.querySelectorAll('.close-order-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const orderId = e.target.getAttribute('data-order-id');
                const currentUser = JSON.parse(localStorage.getItem('currentUser')) || { id: 'default-user-id' };
                fetch(`http://localhost:5500/api/users/${currentUser.id}/orders/${orderId}/close`, {
                    method: 'PUT'
                })
                .then(response => {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // Refresh reservations when switching to reservations tab
    ...
    function displayReservations() {
        ...
        // Filter for active reservations (assuming no cancellation status is stored yet)
